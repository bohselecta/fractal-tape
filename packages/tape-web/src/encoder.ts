...
export function buildTrie(entries: GlyphEntry[]): TrieNode {
  const root: TrieNode = { kids: new Map() };
  for (const e of entries){
    let n = root;
    for (const w of e.phrase){
      if (!n.kids.has(w)) n.kids.set(w, { kids: new Map() });
      n = n.kids.get(w)!;
    }
    n.glyph = e.glyph;
  }
...
export function encodeWithTrie(words: string[], trie: TrieNode): string[] {
  const out: string[] = [];
  for (let i=0;i<words.length;){
    let n = trie, j = i, last: null | { len:number; glyph:string } = null;
    while (j<words.length){
      const kid = n.kids.get(words[j]); if (!kid) break;
      n = kid; if (n.glyph) last = { len: j-i+1, glyph: n.glyph }; j++;
    }
